...
__global__ void bfs_kernel_warp_optimized(
    ...
    int num_vertices)
{
    ...
    int v = blockIdx.x * (blockDim.x/32) + warp_id;

    if(v >= num_vertices) return;

    // Warp-wide bitmap check
    unsigned int mask = __ballot_sync(0xFFFFFFFF,
        frontier_bitmap[v/32] & (1U << (v%32)));
    ...
    // Process neighbors in warp-cooperative fashion
...
double benchmark_bfs_gpu_warp_centric(csr_graph *csr, int source, int* distances) {
    // Device allocations
    ...
    while(active) {
        ...
        // Warp-optimized grid configuration
        dim3 block(256);  // 8 warps per block
        dim3 grid((csr->num_vertices + (block.x/32) - 1) / (block.x/32));

        bfs_kernel_warp_optimized<<<grid, block>>>(
            d_row, d_col, d_dist,
    ...
    printf("\tbenchmarking GPU-Warp-Centric-Breadth-First-Search: %8.4f ms \n", msec_per_iteration);
...
int main(int argc, char** argv) {
    ...
    int source = 0;
    benchmark_bfs_gpu_warp_centric(&csr, source, distances);
